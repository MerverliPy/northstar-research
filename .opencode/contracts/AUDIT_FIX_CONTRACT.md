...
### [ ] AF-001 — Remove hardcoded "northstar" password from Neo4j repo default arg
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-002 — Fix portal agent proxy: add /api/v1 prefix to all agent URLs
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-003 — Fix path traversal in SPA file serving
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-004 — Remove wildcard CORS on SSE chat endpoint
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-005 — Fix LLMResponseCache key: include system_prompt, temperature, max_tokens
...
### [ ] AF-006 — Fix VectorStore.health(): replace heartbeat() with real liveness check
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-007 — Add VectorStore.close() and close it in dependencies.shutdown
...
### [ ] AF-008 — Fix SearchResult.source_id type mismatch (vector vs northstar-models)
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-009 — Fix doctor.sh: use underscores not hyphens for import checks
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-010 — Add missing updated_at columns in migration (4 tables)
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-011 — Add ExtractionLogCreate and ExtractionLogUpdate schemas
...
### [ ] AF-012 — Fix portal proxy: unsilence swallowed exceptions
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-013 — Use embed_batch() in VectorStore.add_documents() instead of serial embed loop
...
### [ ] AF-014 — Fix score calculation: use correct cosine→similarity formula
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-015 — Add retry logic to LLMService.generate() (3 retries, exponential backoff)
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-016 — Add timeout to EmbeddingService httpx client
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-017 — Fix N+1 refresh in bulk_create_entities and bulk_create_claims
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-018 — Add close() to LLMResponseCache and call from LLMService.close()
- **Severity:** MEDIUM
- **Files:**
...
### [ ] AF-019 — Fix update_extraction_log status transition logic
- **Severity:** MEDIUM
- **Files:**
...
### [ ] AF-020 — Add Pydantic Field validators on confidence (0.0–1.0) and quality_score (0.0–1.0)
- **Severity:** MEDIUM
- **Files:**
...
### [ ] AF-021 — Fix restore.sh: add confirmation prompt + auto-backup before restore
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-022 — Fix check-health.sh: don't curl Neo4j Bolt port (use tcp check or skip)
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-023 — Add prerequisite checks to backup.sh (pg_dump availability)
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-024 — Add safety gate to chat-import-bridge (PROMOTION_ENABLED flag)
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-025 — Prevent Header Injection in Portal's Generic API Proxy
- **Severity:** MEDIUM
- **Files:**
...
### [ ] AF-026 — Add `USER appuser` to Dockerfile (don't run as root)
- **Severity:** HIGH
- **Files:**
  - `Dockerfile`
...
### [ ] AF-027 — Add Neo4j health check to docker-compose
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-028 — Add unit tests for agent_tools.py (13 tool methods)
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-029 — Add unit tests for orchestrator.py
- **Severity:** CRITICAL
- **Files:**
...
### [ ] AF-030 — Add unit tests for conversation.py (SQLite chat store)
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-031 — Add unit tests for promotion.py service
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-032 — Un-skip portal page tests (dashboard, quality, cleanup, extraction, graph)
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-033 — Un-skip portal safety gate tests (extraction force-enabled, cleanup enabled)
- **Severity:** HIGH
- **Files:**
...
### [ ] AF-034 — Fix test_cleanup_report_never_modifies: actually call the endpoint
- **Severity:** MEDIUM
- **Files:**
...
### [ ] AF-035 — Fix `alembic.ini` or `env.py` to include `updated_at` for missing tables via new migration
...
### [ ] AF-036 — Reorder schemas: EntityCreate → EntityRead → EntityUpdate
...
### [ ] AF-037 — Fix Entity.aliases from `dict` → `list[str]` across all 3 layers
...
### [ ] AF-038 — Add `ondelete="CASCADE"` to FK relationships in ORM models
...
### [ ] AF-039 — Fix `tools/pyproject.toml` namespace conflict
...
### [ ] AF-040 — Add `restart: unless-stopped` to docker-compose services
...
### [ ] AF-041 — Unify `TEMPLATE_DIR` pattern across portal routers (deduplicate 5 copies)
...
### [ ] AF-042 — Add `ge=1, le=1000` validation on list endpoint limit query params
...
### [ ] AF-043 — Break monolithic portal `api_v1_handler` into route-delegated proxy modules
...
| Band | Tasks | Done |
|------|-------|------|
| BAND 1 — Immediate (Security+Correctness) | AF-001 through AF-012 | 0/12 |
| BAND 2 — Data Integrity+Performance | AF-013 through AF-020 | 0/8 |
| BAND 3 — Safety+Hardening | AF-021 through AF-027 | 0/7 |
| BAND 4 — Test Coverage | AF-028 through AF-034 | 0/7 |
| BAND 5 — Housekeeping | AF-035 through AF-043 | 0/9 |
